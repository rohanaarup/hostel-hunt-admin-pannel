# Hostel Hunt — Data Isolation Design Doc (Phase 1)

Status: **DRAFT — awaiting approval before Phase 2 implementation.**
Scope: `admin pannel HH/backend` (Django REST Framework). No code changed in this phase.
...
7. **Explicit cross-tenant/platform-admin bypass:** add `IsPlatformAdmin` permission class (`request.user.is_superuser`) and a `PlatformAdminQuerysetMixin` that, only when combined explicitly with `IsPlatformAdmin` on a specific view, skips the owner/user filter and logs the access (`logging.getLogger("tenancy.bypass").info(...)` with the requesting admin's ID, model, and pk/queryset). This must be a distinct opt-in mixin an engineer deliberately adds to a specific admin-only view — never a flag that silently widens an existing owner/student view.
...
1. **The "confirmed `OwnerListView` leak"** named in the brief does not exist in the current code, and I found no trace of a prior audit anywhere in the repo. Please point me at the specific endpoint/repro if this bug is still live somewhere I haven't checked (e.g. is it actually in the Next.js admin panel calling the wrong query param, rather than a backend queryset issue?).
2. **`Notice` rows with `hostel=NULL`:** are these real production data (intentional "global" notices), or an artifact of test/seed data? This determines whether the `OWNER_LOOKUP` fix needs the combined `Q()` fallback or can be a straight rename.
3. **Platform-admin bypass:** should this reuse Django's existing `is_superuser` flag (already used for `createsuperowner`/Django admin), or do you want a distinct "platform support" flag/role separate from Django-admin superuser status? Affects whether `IsPlatformAdmin` is one line or needs a new field + migration.
4. **`DashboardStatsView`/`DashboardActivityView`:** confirmed broken against the current `Payment` schema (references fields that don't exist) and confirmed still routed live. Do you want these fixed as part of this pass (in which case they'd need real requirements gathering on what fields should replace `amount_due`/`amount_paid`), or deleted if the Next.js admin panel no longer calls them, or left alone (filed as a separate, non-tenancy bug ticket)? I have not checked the frontend per the backend-only rule, so I can't tell you whether it's dead from that side.
5. **`StudentDashboardStatsView`'s legacy name-match fallback** (`Q(student__isnull=True, student_name__iexact=user_name)`) is a soft cross-user aggregation risk (two students who share a display name would see each other's guest-booking stats folded together). Low severity, aggregate-only, not a detail-level leak — do you want this tightened/removed as part of this pass, or left as-is (it predates this task and may be intentional for a legacy-data-migration reason I don't have context on)?
