...
export default function HostelEnrollmentWizard({ onClose }: Props) {
  ...
  if (submitted) {
    return (
      <div className="bg-ivory-50 dark:bg-ivory-950 border border-ivory-300 dark:border-ivory-700 rounded-2xl w-full max-w-lg mx-auto p-12 text-center">
        ...
        <h2 className="text-2xl font-bold text-ink-900 dark:text-ivory-50 mb-2">Hostel Registered! 🎉</h2>
        <p className="text-ink-700 dark:text-ivory-500 text-sm mb-2">
          <span className="text-ink-900 dark:text-ivory-50 font-semibold">{data.name}</span> has been submitted for review.
        </p>
        <p className="text-ink-700 dark:text-ivory-500 text-xs">Redirecting to your dashboard...</p>
        <div className="mt-6 h-1 bg-ivory-300 dark:bg-ivory-700 rounded-full overflow-hidden">
          <div className="h-full bg-auburn-500 dark:bg-auburn-300 animate-[progress_2.5s_ease-out_forwards]" style={{ width: '0%' }} />
        </div>
      </div>
    );
  }
    ...
    <div className="bg-ivory-50 dark:bg-ivory-950 border border-ivory-300 dark:border-ivory-700 rounded-2xl w-full max-w-3xl mx-auto flex flex-col max-h-[90vh] shadow-2xl">
      ...
      <div className="px-8 pt-7 pb-6 border-b border-ivory-300 dark:border-ivory-700 flex-shrink-0">
        <div className="flex justify-between items-start mb-6">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="text-ink-700 dark:text-ivory-500 hover:text-ink-900 dark:hover:text-ivory-50 transition-colors p-1 flex-shrink-0"
              title="Close"
            >
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 min-h-0">
        ...
        {currentStep === 5 && (
          <div className="flex flex-col gap-4">
            <Step6Review data={data} onEditStep={setCurrentStep} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
            {submitError && (
      ...
      {currentStep < 5 && (
        <div className="px-8 py-5 border-t border-ivory-300 dark:border-ivory-700 flex justify-between items-center flex-shrink-0">
          ...
          <div className="text-[12px] text-ink-700 dark:text-ivory-500 font-medium">
