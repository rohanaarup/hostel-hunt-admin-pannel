...
export default function Step2HostelInfo({ data, onChange, errors }: Props) {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div>
        <h3 className="text-lg font-bold text-ink-900 dark:text-ivory-50 mb-1">Hostel Information</h3>
        <p className="text-ink-700 dark:text-ivory-500 text-sm">Details about capacity, type, and policies.</p>
      </div>
            ...
            <button
              ...
              className={`flex flex-col items-center gap-2 py-4 rounded-[10px] border transition-all font-medium text-sm ${
                data.gender_type === opt.value
                  ? 'border-auburn-500 bg-auburn-500/10 text-ink-900 dark:border-auburn-300 dark:bg-auburn-300/10 dark:text-ivory-50'
                  : 'border-ivory-300 bg-ivory-50 text-ink-700 hover:border-ivory-400 hover:text-ink-900 dark:border-ivory-700 dark:bg-ivory-950 dark:text-ivory-500 dark:hover:border-ivory-600 dark:hover:text-ivory-50'
              }`}
            >
      ...
      {/* Occupancy types */}
      <FormField label="Occupancy Types" required error={errors.occupancy_types as string} hint="Select all that apply">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-[8px] text-sm font-semibold border transition-all ${
                data.occupancy_types.includes(opt.value)
                  ? 'border-auburn-500 bg-auburn-500/15 text-auburn-500 dark:border-auburn-300 dark:bg-auburn-300/15 dark:text-auburn-300'
                  : 'border-ivory-300 bg-ivory-50 text-ink-700 hover:border-ivory-400 dark:border-ivory-700 dark:bg-ivory-950 dark:text-ivory-500 dark:hover:border-ivory-600'
              }`}
            >
              {opt.label}
            </button>
