...
export default function ForgotPasswordPage() {
  ...
  const inputBase = () =>
    'w-full bg-ivory-50 dark:bg-ivory-900 border border-ivory-300 dark:border-ivory-700 rounded-[10px] px-4 py-3 text-ink-900 dark:text-ivory-50 placeholder-ink-700 dark:placeholder-ivory-500 outline-none transition-all font-medium text-sm focus:border-auburn-500 dark:focus:border-auburn-300 focus:ring-[3px] focus:ring-auburn-500/20 dark:focus:ring-auburn-300/20';
  ...
  const ErrorBanner = () => error ? (
    <div className="mb-4 bg-error/10 border border-error/20 text-error text-sm rounded-lg p-3 flex gap-2 items-start">
  ...
  const SpinBtn = ({ label, loading }: { label: string; loading?: boolean }) => (
    <button type="submit" disabled={loading}
      className="w-full bg-auburn-500 hover:bg-auburn-700 dark:bg-auburn-300 dark:hover:bg-auburn-100 text-ivory-50 dark:text-ink-900 font-semibold py-3 rounded-[10px] transition-all auburn-glow disabled:opacity-60 flex items-center justify-center gap-2">
      {loading ? (
    ...
    <AuthLayout heroTitle={'Reset your\npassword securely'}>
      <div className="bg-ivory-100 dark:bg-ivory-900/80 rounded-2xl border border-ivory-300 dark:border-ivory-700 p-8 shadow-2xl relative overflow-hidden glass-card">
        ...
        {step === 'identifier' && (
          <>
            <div className="mb-6 relative z-10">
              <h2 className="text-2xl font-bold text-ink-900 dark:text-ivory-50">Forgot password?</h2>
              <p className="text-ink-700 dark:text-ivory-500 mt-1 text-sm">Enter your registered email or phone</p>
            </div>
            <ErrorBanner />
                ...
                {(['email', 'phone'] as IdentifierType[]).map(t => (
                  <button key={t} type="button"
                    ...
                    className={`flex-1 py-1.5 text-xs font-semibold rounded-[8px] transition-all ${
                      identifierType === t ? 'bg-auburn-500 text-ivory-50 dark:bg-auburn-300 dark:text-ink-900' : 'text-ink-700 dark:text-ivory-500 hover:text-ink-900 dark:hover:text-ivory-50'
                    }`}>
                    {t === 'email' ? '✉ Email' : '📱 Phone'}
                  </button>
                ))}
            ...
            <div className="mt-6 text-center text-[13px] text-ink-700 dark:text-ivory-500 relative z-10">
              <Link href="/login" className="text-auburn-500 hover:text-auburn-700 dark:text-auburn-300 dark:hover:text-auburn-100 font-semibold">← Back to Login</Link>
            </div>
          </>
        )}
        ...
        {step === 'otp' && (
          <>
            <div className="mb-6 relative z-10 text-center">
              <h2 className="text-2xl font-bold text-ink-900 dark:text-ivory-50">Enter OTP</h2>
              <p className="text-ink-700 dark:text-ivory-500 mt-1 text-sm">
                Code sent to <span className="text-ink-900 dark:text-ivory-50 font-semibold">{identifier}</span>
              </p>
            </div>
            <ErrorBanner />
            <form onSubmit={handleVerifyOtp} className="space-y-6 relative z-10">
              ...
              <div className="text-center">
                {timerActive ? (
                  <p className="text-ink-700 dark:text-ivory-500 text-sm">Resend in <span className="text-auburn-500 dark:text-auburn-300 font-bold tabular-nums">{countdown}s</span></p>
                ) : (
                  ...
                  }} className="text-auburn-500 hover:text-auburn-700 dark:text-auburn-300 dark:hover:text-auburn-100 text-sm font-semibold">Resend OTP</button>
                )}
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => { setStep('identifier'); setError(null); setOtp(''); }}
                  className="flex-1 py-3 border border-ivory-300 dark:border-ivory-700 hover:border-ivory-500 dark:hover:border-ivory-500 text-ink-700 dark:text-ivory-500 hover:text-ink-900 dark:hover:text-ivory-50 rounded-[10px] font-medium text-sm transition-all">← Back</button>
                <button type="submit" disabled={isLoading || otp.length < 6}
                  className="flex-1 bg-auburn-500 hover:bg-auburn-700 dark:bg-auburn-300 dark:hover:bg-auburn-100 text-ivory-50 dark:text-ink-900 font-semibold py-3 rounded-[10px] transition-all auburn-glow disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        {step === 'new_password' && (
          <>
            <div className="mb-6 relative z-10">
              <h2 className="text-2xl font-bold text-ink-900 dark:text-ivory-50">New password</h2>
              <p className="text-ink-700 dark:text-ivory-500 mt-1 text-sm">Choose a strong new password</p>
            </div>
            <ErrorBanner />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" tabIndex={-1} onClick={() => setShowPwd(p => !p)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-700/50 hover:text-ink-700 dark:text-ivory-500/50 dark:hover:text-ivory-500 p-1">
        ...
        {step === 'success' && (
          <div className="text-center py-8 relative z-10">
            ...
            <h2 className="text-2xl font-bold text-ink-900 dark:text-ivory-50 mb-2">Password reset!</h2>
            <p className="text-ink-700 dark:text-ivory-500 text-sm mb-8">Your password has been updated successfully.</p>
            <button onClick={() => router.push('/login')}
              className="bg-auburn-500 hover:bg-auburn-700 dark:bg-auburn-300 dark:hover:bg-auburn-100 text-ivory-50 dark:text-ink-900 font-semibold px-8 py-3 rounded-[10px] transition-all auburn-glow">
              Sign In Now
            </button>
          </div>
        )}
