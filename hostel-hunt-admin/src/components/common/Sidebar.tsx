...
const NAV_ITEMS = [
  {
    ...
    items: [
      ...
      { 
        ...
        subGroups: [
          {
            label: 'Offline Booking Requests',
            items: [
              { to: '/bookings?tab=offline-requests', label: 'Requests' },
              { to: '/bookings?tab=offline-confirmed', label: 'Confirmed' }
            ]
          },
          {
            label: 'Online Booking Requests',
            items: [
              { to: '/bookings?tab=online-requests', label: 'Requests' },
              { to: '/bookings?tab=online-payments', label: 'Payments Received (Coming Soon)' }
            ]
          }
        ]
      },
        ...
        {NAV_ITEMS.map(group => (
          <div key={group.label} className="mb-2">
            ...
            {group.items.map(item => {
              ...
              return (
                <div key={item.to}>
                  <NavLink to={item.subGroups ? item.subGroups[0].items[0].to : item.to} className={() => navItemClass(isItemActive)}>
                  ...
                  {item.subGroups && isItemActive && (
                    <div className="ml-9 mt-2 mb-3 space-y-4 border-l border-[#2A2A2A] pl-4">
                      {item.subGroups.map(subGroup => (
                        <div key={subGroup.label}>
                          <div className="text-[10px] text-[#6B6B6B] font-bold mb-1.5 uppercase tracking-wider">{subGroup.label}</div>
                          <div className="space-y-1.5">
                            {subGroup.items.map(subItem => {
                              const searchParams = new URLSearchParams(location.search);
                              const subItemSearch = new URLSearchParams(subItem.to.split('?')[1] || '');
                              const isSubActive = searchParams.get('tab') === subItemSearch.get('tab');
                              
                              return (
                                <NavLink 
                                  key={subItem.to} 
                                  to={subItem.to} 
                                  className={`block text-[12px] py-1 transition-colors ${isSubActive ? 'text-[#E8571A] font-semibold' : 'text-[#9A9690] hover:text-white'}`}
                                >
                                  • {subItem.label}
                                </NavLink>
                              );
